...
const paths = require('./webpack-config/base/paths.config.js'); // 与业务代码共用同一份路径的配置表
...
const configDll = {
	...
	plugins: [
		new webpack.DllPlugin({
			...
			context: paths.rootDir, // 指定一个路径作为上下文环境，需要与DllReferencePlugin的context参数保持一致，建议统一设置为项目根目录
		}),
		/* 跟业务代码一样，该兼容的还是得兼容 */
	...
	module: require('./webpack-config/module.prod.config.js'), // 沿用业务代码的module配置
